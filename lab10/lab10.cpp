...
int main(){
    ...
    while(scanf("%d %d",&processor_num,&task_num) == 2){
		...
		//count rate and U
		for(i = 0 ; i < task_num ; i++){
			task[i].rate = 1/(float)task[i].period;
			task[i].U = (float)task[i].execution_time/task[i].period;
			U_total += task[i].U;

		}
		Urm = (float)task_num*(pow(2,(float)1/task_num) -1);
		if(U_total <= Urm){
			cout <<"------------------------------------------"<<endl;
			cout << "Test case "<<Case<<" : schedulable"<<endl;
			cout <<"------------------------------------------"<<endl;
			RM_P(processor,task,processor_num,task_num);
		}
		else{
		    cout <<"------------------------------------------"<<endl;
